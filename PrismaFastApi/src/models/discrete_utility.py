...
class DiscreteUtilityParentOutcome(Base):
    ...
    discrete_utility_id: Mapped[uuid.UUID] = mapped_column(
        GUID(), ForeignKey("discrete_utility.id"), primary_key=True
    )
    parent_outcome_id: Mapped[uuid.UUID] = mapped_column(
        GUID(), ForeignKey("outcome.id", ondelete="CASCADE"), primary_key=True
    )

    discrete_utility: Mapped["DiscreteUtility"] = relationship(
        "DiscreteUtility", back_populates="parent_outcomes"
    )
...
class DiscreteUtilityParentOption(Base):
    ...
    parent_option_id: Mapped[uuid.UUID] = mapped_column(
        GUID(), ForeignKey("option.id", ondelete="CASCADE"), primary_key=True
    )

    discrete_utility: Mapped["DiscreteUtility"] = relationship(
        "DiscreteUtility", back_populates="parent_options"
    )
...
class DiscreteUtility(Base, BaseEntity):
    ...
    value_metric_id: Mapped[uuid.UUID] = mapped_column(
        GUID(), ForeignKey("value_metric.id", ondelete="CASCADE"), index=True
    )
    utility_id: Mapped[uuid.UUID] = mapped_column(
        GUID(), ForeignKey("utility.id"), index=True
    )  # cascade delete handled in utility model
    utility_value: Mapped[Optional[float]] = mapped_column(
        Float(precision=DatabaseConstants.FLOAT_PRECISION.value), default=None, nullable=True
    )

    value_metric: Mapped["ValueMetric"] = relationship(
        "ValueMetric", foreign_keys=[value_metric_id]
    )
    utility: Mapped["Utility"] = relationship(
        "Utility", back_populates="discrete_utilities", foreign_keys=[utility_id]
    )
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return (
            self.id == other.id
            and self.utility_id == other.utility_id
            and self.value_metric_id == other.value_metric_id
            and self.utility_value == other.utility_value
            and len(self.parent_outcomes) == len(other.parent_outcomes)
            and len(self.parent_options) == len(other.parent_options)
            and all(
                po1.parent_outcome_id == po2.parent_outcome_id
                for po1, po2 in zip(
            ...
            and all(
                po1.parent_option_id == po2.parent_option_id
                for po1, po2 in zip(
