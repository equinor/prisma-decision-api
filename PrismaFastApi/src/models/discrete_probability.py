...
class DiscreteProbabilityParentOutcome(Base):
    ...
    discrete_probability_id: Mapped[uuid.UUID] = mapped_column(
        GUID(), ForeignKey("discrete_probability.id"), primary_key=True
    )
    parent_outcome_id: Mapped[uuid.UUID] = mapped_column(
        GUID(), ForeignKey("outcome.id", ondelete="CASCADE"), primary_key=True
    )

    discrete_probability: Mapped["DiscreteProbability"] = relationship(
        "DiscreteProbability", back_populates="parent_outcomes"
    )
...
class DiscreteProbabilityParentOption(Base):
    ...
    discrete_probability: Mapped["DiscreteProbability"] = relationship(
        "DiscreteProbability", back_populates="parent_options"
    )
    parent_option: Mapped["Option"] = relationship(
        "Option",
    )
...
class DiscreteProbability(Base, BaseEntity):
    ...
    outcome_id: Mapped[uuid.UUID] = mapped_column(
        GUID(), ForeignKey("outcome.id", ondelete="CASCADE"), index=True
    )
    uncertainty_id: Mapped[uuid.UUID] = mapped_column(
        GUID(), ForeignKey("uncertainty.id"), index=True
    )  # cascade delete handled in uncertainty model
    ...
    uncertainty: Mapped["Uncertainty"] = relationship(
        "Uncertainty", back_populates="discrete_probabilities", foreign_keys=[uncertainty_id]
    )
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return (
            self.id == other.id
            and self.uncertainty_id == other.uncertainty_id
