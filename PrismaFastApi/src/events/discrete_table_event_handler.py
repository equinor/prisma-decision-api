...
from src.utils.session_info_handler import SessionInfoHandler, SessionInfo
...
class DiscreteTableEventHandler:
    ...
    def process_session_changes_before_flush(self, session: Session) -> None:
        ...
        subscribed_dirty = [
            ...
            if any(
                isinstance(entity, entity_type)
                for entity_type in self.subscribed_entities_modified_before_flush
            )
        ]
        subscribed_deleted = [
            entity
            ...
            if any(
                isinstance(entity, entity_type) for entity_type in self.subscribed_entities_delete
            )
        ]
        ...
        if subscribed_deleted:
            session_info = SessionInfoHandler.add_to_session_info(
                session_info, self._process_deletions(session, subscribed_deleted)
            )

        if subscribed_dirty:
            session_info = SessionInfoHandler.add_to_session_info(
                session_info, self._process_modifications(session, subscribed_dirty)
            )
    ...
    def process_session_changes_after_flush(self, session: Session) -> None:
        ...
        if subscribed_new:
            # Process changes in order of dependency
            session_info = SessionInfoHandler.add_to_session_info(
                session_info, self._process_additions(session, subscribed_new)
            )
    ...
    def _process_deletions(self, session: Session, deleted_entities: list[Any]) -> SessionInfo:
        ...
        for deleted_entity in deleted_entities:
            ...
            elif isinstance(deleted_entity, DiscreteProbabilityParentOutcome) or isinstance(
                deleted_entity, DiscreteProbabilityParentOption
            ):
                discrete_probabilities_to_delete.add(deleted_entity.discrete_probability_id)
            elif isinstance(deleted_entity, DiscreteUtilityParentOutcome) or isinstance(
                deleted_entity, DiscreteUtilityParentOption
            ):
                discrete_utilities_to_delete.add(deleted_entity.discrete_utility_id)

        if discrete_probabilities_to_delete:
            session.execute(
                DiscreteProbabilityParentOutcome.__table__.delete().where(
                    DiscreteProbabilityParentOutcome.discrete_probability_id.in_(
                        discrete_probabilities_to_delete
                    )
                )
            )
            ...
            session.execute(
                DiscreteProbabilityParentOption.__table__.delete().where(
                    DiscreteProbabilityParentOption.discrete_probability_id.in_(
                        discrete_probabilities_to_delete
                    )
                )
            )
            ...
            session.execute(
                DiscreteProbability.__table__.delete().where(
                    DiscreteProbability.id.in_(discrete_probabilities_to_delete)
                )
            )

        if discrete_utilities_to_delete:
            session.execute(
                DiscreteUtilityParentOutcome.__table__.delete().where(
                    DiscreteUtilityParentOutcome.discrete_utility_id.in_(
                        discrete_utilities_to_delete
                    )
                )
            )
            ...
            # Then delete all parent option relationships
            session.execute(
                DiscreteUtilityParentOption.__table__.delete().where(
                    DiscreteUtilityParentOption.discrete_utility_id.in_(
                        discrete_utilities_to_delete
                    )
                )
            )
            ...
            session.execute(
                DiscreteUtility.__table__.delete().where(
                    DiscreteUtility.id.in_(discrete_utilities_to_delete)
                )
            )
    ...
    def _process_modifications(self, session: Session, modified_entities: list[Any]) -> SessionInfo:
        ...
        if edge_tail_to_head_mapping or head_ids:
            session_info = SessionInfoHandler.add_to_session_info(
                session_info,
                node_repository.add_effected_session_enities_by_nodes(
                    session, head_ids, edge_tail_to_head_mapping
                ),
            )
    ...
    def _process_additions(self, session: Session, new_entities: list[Any]) -> SessionInfo:
        ...
        if added_edges:
            session_info = SessionInfoHandler.add_to_session_info(
                session_info, edge_repository.find_effected_session_entities(session, added_edges)
            )
    ...
    def _has_boundary_change(self, issue: Issue) -> bool:
        ...
        if not history.has_changes():
            return False
        return Boundary.OUT.value in (history.added or []) or Boundary.OUT.value in (
            history.deleted or []
        )
    ...
    def _has_focus_type_change(self, decision: Decision) -> bool:
        ...
        if not history.has_changes():
            return False
        return DecisionHierarchy.FOCUS.value in (
            history.added or []
        ) or DecisionHierarchy.FOCUS.value in (history.deleted or [])
    ...
    def remove_options_from_strategy_table(session: Session):
        session_info = SessionInfoHandler.get_session_info(session)
        strategy_repository.remove_strategy_options_out_of_scope(
            session, session_info.issues_pending_strategy_removal
        )
