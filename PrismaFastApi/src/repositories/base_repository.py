...
class BaseRepository(Generic[T, IDType]):
    ...
    async def _replace_strategy_options(
        self, strategy_to_update: Strategy, new_strategy_options: list[StrategyOption]
    ) -> None:
        """
        ...
        for new_strategy_option in new_strategy_options:
            strategy_option_to_add = StrategyOption(
                strategy_id=strategy_to_update.id, option_id=new_strategy_option.option_id
            )
    ...
    async def _update_strategy(self, incoming_entity: Strategy, existing_entity: Strategy):
        ...
        existing_entity.icon_color = incoming_entity.icon_color
    ...
    async def _update_strategies(
        ...
        if strategies_to_delete:
            ...
            await asyncio.gather(
                *[self.session.delete(strategy) for strategy in strategies_to_delete]
            )
    ...
    async def _update_project_roles(
        ...
        roles_to_delete = [role for role in existing_entities if role.id not in incoming_by_id]
        if roles_to_delete:
        ...
        new_roles = [role for role in incoming_entities if role.id not in existing_by_id]
        if new_roles:
