...
from src.models import Issue, Node, Edge, Decision


def uncertainty_table_load_query(id: uuid.UUID):
    return (
        select(Uncertainty)
        .where(Uncertainty.id == id)
        .options(
            ...
            joinedload(Uncertainty.issue).options(
                joinedload(Issue.node).options(
                    subqueryload(Node.head_edges).options(
                        joinedload(Edge.tail_node).options(joinedload(Node.issue))
                    ),
                ),
...
def perform_recalc(entity: Uncertainty) -> Uncertainty:
    ...
    edges = list(
        {(edge.tail_id, edge.head_id): edge for edge in entity.issue.node.head_edges}.values()
    )
    for edge in edges:
        issue = edge.tail_node.issue
        if issue.boundary not in [Boundary.IN.value, Boundary.ON.value]:
            continue

        if issue.type == Type.UNCERTAINTY:
            # check that this is a key uncertainty
            if not issue.uncertainty or not issue.uncertainty.is_key:
                continue
            ...
            if not issue.decision or issue.decision.type != DecisionHierarchy.FOCUS.value:
                continue
    ...
    if len(parent_outcomes_list) == 0 and len(parent_options_list) == 0:
        entity.discrete_probabilities = [
            DiscreteProbability(
                id=uuid.uuid4(), uncertainty_id=entity.id, outcome_id=x.id, probability=0
            )
            for x in entity.outcomes
        ]
        return entity
        ...
        for parent_combination in parent_combinations:
            ...
            entity.discrete_probabilities.append(
                DiscreteProbability(
                    ...
                    parent_outcomes=[
                        DiscreteProbabilityParentOutcome(
                            discrete_probability_id=probability_id, parent_outcome_id=x
                        )
                        for x in parent_outcome_ids
                    ],
                    parent_options=[
                        DiscreteProbabilityParentOption(
                            discrete_probability_id=probability_id, parent_option_id=x
                        )
                        for x in parent_option_ids
                    ],
                )
            )
...
def recalculate_discrete_probability_table(
