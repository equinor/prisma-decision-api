...
class DiscreteProbabilityService:
    async def create(
        ...
        result: list[DiscreteProbabilityOutgoingDto] = DiscreteProbabilityMapper.to_outgoing_dtos(
            entities
        )
        return result
    ...
    async def get(
        ...
        discrete_probabilities: list[DiscreteProbability] = await DiscreteProbabilityRepository(
            session
        ).get(ids)
    ...
    async def get_all(
        ...
        discrete_probabilities: list[DiscreteProbability] = await DiscreteProbabilityRepository(
            session
        ).get_all(odata_query=odata_query)
        result = DiscreteProbabilityMapper.to_outgoing_dtos(discrete_probabilities)
        return result
