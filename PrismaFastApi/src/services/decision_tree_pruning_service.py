...
class TreePruner(ABC):
    ...
    @abstractmethod
    def visit_decision_node(
        self, node: DecisionTreeDto, context: PruningContext
    ) -> Optional[DecisionTreeDto]:
        """Visit and prune a decision node."""
        pass

    @abstractmethod
    def visit_uncertainty_node(
        self, node: DecisionTreeDto, context: PruningContext
    ) -> Optional[DecisionTreeDto]:
    ...
    def prune(self, node: DecisionTreeDto, context: PruningContext) -> Optional[DecisionTreeDto]:
        ...
        if (
            node.tree_node.issue.type == Type.DECISION.value
            and node.tree_node.issue.decision is not None
            and node.tree_node.children
        ):
...
class OptimalDecisionTreePruner(TreePruner):
    ...
    def visit_decision_node(
        ...
        if (
            isinstance(node.tree_node.issue, EndPointNodeDto)
            or not node.tree_node.children
            or not node.tree_node.issue.decision
        ):
    ...
    def visit_uncertainty_node(
        ...
        if (
            isinstance(node.tree_node.issue, EndPointNodeDto)
            or not node.tree_node.children
            or not node.tree_node.issue.uncertainty
        ):
        ...
        for child, outcome in zip(
            node.tree_node.children, node.tree_node.issue.uncertainty.outcomes
        ):
    ...
    def _find_optimal_decision(
        self, node: DecisionTreeDto, optimal_decisions: list[OptimalOption], context: PruningContext
    ) -> Optional[OptimalOption]:
        """Find the optimal decision for the current node given the current path."""
        for decision in optimal_decisions:
            if decision.decision_id == node.tree_node.issue.id and context.is_parent_valid(
                decision
            ):
                return decision
        return None
    ...
    def _find_option_index(self, node: DecisionTreeDto, decision_state_id: uuid.UUID) -> int:
        ...
        if isinstance(node.tree_node.issue, EndPointNodeDto) or not node.tree_node.issue.decision:
            raise DecisionTreePruningException("Invalid decision node visited")
    ...
    def _prune_decision_branches(
        self, node: DecisionTreeDto, option_index: int, decision_state_id: uuid.UUID
    ) -> None:
        ...
        node.tree_node.issue.decision.options = [
            option
            for option in node.tree_node.issue.decision.options
    ...
    def _align_probabilities_with_outcomes(node: DecisionTreeDto) -> None:
        ...
        if (
            isinstance(node.tree_node.issue, EndPointNodeDto)
            or not node.tree_node.issue.uncertainty
            or not node.tree_node.probabilities
        ):
            raise DecisionTreePruningException("Invalid uncertainty node visited")

        outcome_order = {
            outcome.id: i for i, outcome in enumerate(node.tree_node.issue.uncertainty.outcomes)
        }
    ...
    def _remove_zero_probability_outcomes(self, node: DecisionTreeDto) -> None:
        ...
        items_to_remove: list[tuple[DecisionTreeDto, ProbabilityDto, OutcomeOutgoingDto]] = []
...
class DecisionTreePruningService:
    ...
    def prune_tree_for_optimal_decisions(
        self, tree_root: DecisionTreeDto, solution: SolutionDto
    ) -> Optional[DecisionTreeDto]:
...
class TreePrunerOld(ABC):
    ...
    @abstractmethod
    def visit_decision_node(
        self, node: DecisionTreeDtoOld, context: PruningContext
...
class OptimalDecisionTreePrunerOld(TreePrunerOld):
    ...
    def visit_uncertainty_node(
        ...
        outcome: OutcomeOutgoingDto
        for child, outcome in zip(node.children, node.tree_node.issue.uncertainty.outcomes):
            context.add_to_path(outcome.id)
            try:
                self.prune(child, context)
            finally:
    ...
    def _prune_decision_branches(
        self, node: DecisionTreeDtoOld, option_index: int, decision_state_id: uuid.UUID
    ) -> None:
        """Remove non-optimal branches from the decision node."""
        if (
            isinstance(node.tree_node.issue, EndPointNodeDto)
    ...
    def _remove_zero_probability_outcomes(self, node: DecisionTreeDtoOld) -> None:
        ...
        items_to_remove: list[tuple[DecisionTreeDtoOld, ProbabilityDto, OutcomeOutgoingDto]] = []
        ...
        for child, prob, outcome in zip(
            node.children, node.tree_node.probabilities, node.tree_node.issue.uncertainty.outcomes
        ):
            if prob.probability_value == 0:
                items_to_remove.append((child, prob, outcome))
...
class DecisionTreePruningServiceOld:
    ...
    def prune_tree_for_optimal_decisions(
        self, tree_root: DecisionTreeDtoOld, solution: SolutionDto
    ) -> Optional[DecisionTreeDtoOld]:
        """Main method to prune a decision tree for optimal decisions."""
        context = PruningContext(current_path=set(), solution=solution)
        return self.pruner.prune(tree_root, context)
