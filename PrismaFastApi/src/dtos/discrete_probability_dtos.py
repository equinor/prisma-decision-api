...
class DiscreteProbabilityMapper:
    ...
    def to_outgoing_dto(entity: DiscreteProbability) -> DiscreteProbabilityOutgoingDto:
        return DiscreteProbabilityOutgoingDto(
            ...
            parent_outcome_ids=(
                [x.parent_outcome_id for x in entity.parent_outcomes]
                if entity.parent_outcomes
                else []
            ),
            parent_option_ids=(
                [x.parent_option_id for x in entity.parent_options] if entity.parent_options else []
            ),
        )
    ...
    def to_entity(dto: DiscreteProbabilityIncomingDto) -> DiscreteProbability:
        return DiscreteProbability(
            ...
            parent_outcomes=[
                DiscreteProbabilityParentOutcome(
                    discrete_probability_id=dto.id, parent_outcome_id=x
                )
                for x in dto.parent_outcome_ids
            ],
            parent_options=[
                DiscreteProbabilityParentOption(discrete_probability_id=dto.id, parent_option_id=x)
                for x in dto.parent_option_ids
            ],
        )

    @staticmethod
    def to_outgoing_dtos(
        entities: List[DiscreteProbability],
