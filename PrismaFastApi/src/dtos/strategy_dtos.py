...
class StrategyDto(BaseModel):
    ...
    icon_color: Annotated[str, Field(max_length=DatabaseConstants.MAX_LONG_STRING_LENGTH.value)] = (
        ""
    )
...
class StrategyMapper:
    ...
    def to_entity(dto: StrategyIncomingDto, user_id: int) -> Strategy:
        res = Strategy(
            ...
            user_id=user_id,
            strategy_options=[
                StrategyOption(option_id=option.id, strategy_id=dto.id) for option in dto.options
            ],
        )
        return res
