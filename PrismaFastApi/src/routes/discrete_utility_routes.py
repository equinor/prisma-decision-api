...
async def get_discrete_utility(
    ...
    try:
        discrete_utilities: list[DiscreteUtilityOutgoingDto] = await discrete_utility_service.get(
            session, [id]
        )
...
async def get_all_discrete_utility(
    ...
    try:
        discrete_utilities: list[DiscreteUtilityOutgoingDto] = (
            await discrete_utility_service.get_all(session, odata_query=filter)
        )
