...
async def get_discrete_probability(
    id: uuid.UUID,
    discrete_probability_service: DiscreteProbabilityService = Depends(
        get_discrete_probability_service
    ),
    session: AsyncSession = Depends(get_db),
) -> DiscreteProbabilityOutgoingDto:
    try:
        discrete_probabilities: list[DiscreteProbabilityOutgoingDto] = (
            await discrete_probability_service.get(session, [id])
        )
...
async def get_all_discrete_probability(
    ...
    try:
        discrete_probabilities: list[DiscreteProbabilityOutgoingDto] = (
            await discrete_probability_service.get_all(session, odata_query=filter)
        )
