...
async def call_ms_graph_api_users(token: str, username: str) -> list[UserIncomingDto]:
    ...
    async with AsyncClient() as client:
        ...
        if obo_response.is_success:
            ...
            try:
                next_link = (
                    f'https://graph.microsoft.com/v1.0/users?$search="displayName:{username}"'
                )
                access_token = obo_response.json()["access_token"]
                ...
                filtered_users = [
                    ...
                    if not (
                        ...
                        and "Room" in user.get("displayName", "")
                        and "room" not in user.get("displayName", "")
                    )
                ]
            ...
            # Note: The /users endpoint returns a list, so you may need to handle this differently
            # For now, returning the first user or all users depending on your requirements
            except Exception as e:
                raise HTTPException(
                    status_code=500,
                    detail=f"Unexpected error in call_ms_graph_api: {str(e)}",
