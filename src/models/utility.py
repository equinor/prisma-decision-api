...
class Utility(Base, BaseEntity):
    ...
    def __init__(
        ...
        id: uuid.UUID,
        issue_id: uuid.UUID,
        discrete_utilities: Optional[list["DiscreteUtility"]] = None,
    ):
        self.id = id
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return (
            ...
            and len(self.discrete_utilities) == len(other.discrete_utilities)
            and all(
                dp1 == dp2
                for dp1, dp2 in zip(
                    sorted(self.discrete_utilities, key=lambda x: x.id),
                    sorted(other.discrete_utilities, key=lambda x: x.id),
                )
            )
        )
