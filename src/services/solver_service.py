...
class SolverService:
    ...
    async def find_optimal_decision_pyagrum_from_dtos(self, issues: list[IssueOutgoingDto] = [], edges: list[EdgeOutgoingDto] = []):
        solution = await PyagrumSolver().find_optimal_decisions(issues=issues, edges=edges)
    ...
    async def get_decision_tree_for_optimal_decisions_from_dtos(self, project_id: uuid.UUID, issues: list[IssueOutgoingDto] = [], edges: list[EdgeOutgoingDto] = []):
        solution = await PyagrumSolver().find_optimal_decisions(issues=issues, edges=edges)
